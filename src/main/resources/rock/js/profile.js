...
async function fetchAllUserPosts() {
  ...
  const [blogs, music, videos] = await Promise.all([
    fetchData("http://localhost:8080/api/blogs/my-blogs"),
    fetchData("http://localhost:8080/api/music/my-music"),
    fetchData("http://localhost:8080/api/video/my-video"),
  ]);
...
function deletePost(type, postId) {
  ...
  fetch(`http://localhost:8080/api/${type}/${postId}`, {
    method: "DELETE",
